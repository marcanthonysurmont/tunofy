<section
    class="w-full max-w-[1728px] mt-32 md:mt-48 lg:mt-64 mx-auto flex-col flex items-center justify-center mb-32 px-8"
    id="faq">
        ...
        @foreach ($faqs as $faq)
            <div x-data="{ open: false }" class="border-b rounded-lg border-card-stroke py-4 transition-all duration-500">
                <button @click="open = !open"
                    ...
                    <p class="font-headings text-lg md:text-xl font-light">{{ $faq['question'] }}</p>
                ...
                <div class="relative overflow-hidden transition-all max-h-0 duration-400" x-ref="container"
                    ...
                    <div class="px-4 pb-4 max-w-7xl">
                        <p class="text-base font-normal">{{ $faq['answer'] }}</p>
                    </div>
                </div>
            </div>
        @endforeach
